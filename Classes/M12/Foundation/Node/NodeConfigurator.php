...
class NodeConfigurator
{
     ...
     */
    protected function configureCreateAssistanceChildNodes(
        NodeInterface $node,
        NodeType $nodeType,
        array $config,
        $args = []
    ...
    protected function createNodes(
        ...
        foreach ($nodesToCreate as $config) {
            if (false === isset($config['nodeType'])) {
                throw new NodeConfigurationException("Missing `nodeType` to create for node `{$nodeType->getName()}`.",
                    1432821591);
            }
     ...
     */
    protected function configureLinkingButton(
        NodeInterface $node,
        NodeType $nodeType,
        array $config,
        $buttonLinkPropertyName
    ) {
